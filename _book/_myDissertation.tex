...
\chapter{\texorpdfstring{Velocity (\(v\)): rate-of-change of system
trajectory as a dimension reduction technique and potential indicator of
abrupt
change}{Velocity (v): rate-of-change of system trajectory as a dimension reduction technique and potential indicator of abrupt change}}\label{velocity}
...
\section{Example system}\label{example-system}
...
\caption{The 2-variable toy system used to demonstrate steps for calculating system velocity. Each variable, $x$, is drawn from a normal distribution with means that change at $t = 50$, and constant variance.}\label{fig:sysEx}
\end{figure}
\section{\texorpdfstring{Steps for calculating system velocity,
\(v\)}{Steps for calculating system velocity, v}}\label{steps-for-calculating-system-velocity-v}
...
time points \(t = 1\) \& \(t=2\) (Fig. \ref{fig:sysEx2}). In this
example, the change in values for state variable \(x_1\) from time point
1 to 2, is:
\begin{equation}
\begin{array}{rcr}
...
\(\frac {\Delta s}{\Delta t}\)), by first calculating the change in
\(s\) (Eq. \eqref{eq:s}), and then divide by the total time elapsed
...
\end{tabular}
\end{table}
\section{Velocity as an indicator of abrupt changes in means and
variance}\label{velocity-as-an-indicator-of-abrupt-changes-in-means-and-variance}

Variance is a commonly-used indicator of ecological regime shifts,
however, fails to perform when the number of variables is
\textgreater{}\textgreater{} a few.
\begin{figure}
...
\caption{System change ($s$) and velocity ($v$) of the model system over the time period. Variance equal to mean ($\bar{x_i}=\sigma x_i$), where means ($\bar{x}_{1_{pre}}=25$, $\bar{x}_{1_{post}}=50$, $\bar{x}_{2_{pre}}=15$, $\bar{x}_{2_{post}}=150$).}\label{fig:velocSysEx4}
\end{figure}
