...
ecological regime shift detection methods (hereafter, RDMs) would
require little knowledge of the intrinsic drivers of the system, and the
users of the method would not be required to know if and where a regime
...
the current state of RDMs as being either system--specific (i.e., the
...
capture, but is also notoriously imperfect---that is, process and
...
Information metric. First introduced in by Fath, Cabezas, \& Pawlowski
(2003) as one of multiple steps in calculating their variant of Fisher
...
\chapter*{R package bbsRDM}\label{bbsRDM}
\addcontentsline{toc}{chapter}{R package bbsRDM}

Placeholder

\section{Load packages \& create local
directories}\label{load-packages-create-local-directories}

\section{Download the BBS data and save to file
locally}\label{download-the-bbs-data-and-save-to-file-locally}

\section{Create the sampling grid}\label{create-the-sampling-grid}

\section{Next, we can subset the BBS data by species and/or functional
traits (OPTIONAL but
recommended)}\label{next-we-can-subset-the-bbs-data-by-species-andor-functional-traits-optional-but-recommended}

\section{Subset species according to AOU species codes (i.e.~by family,
genera,
etc..)}\label{subset-species-according-to-aou-species-codes-i.e.by-family-genera-etc..}

\section{Subset species by trait, body mass, taxonomically,
etc\ldots{}}\label{subset-species-by-trait-body-mass-taxonomically-etc}

\section{Calculate regime detection metrics across space or
time}\label{calculate-regime-detection-metrics-across-space-or-time}

\subsection{First, define the parameters required to calcualte the
metrics.}\label{first-define-the-parameters-required-to-calcualte-the-metrics.}

\section{Define the years we want to
analyze}\label{define-the-years-we-want-to-analyze}

\section{Conduct analysis!}\label{conduct-analysis}

\section{Import and munge the results to prepare for
visualization}\label{import-and-munge-the-results-to-prepare-for-visualization}

\section{Visualize results: one regime detection metric at a
time}\label{visualize-results-one-regime-detection-metric-at-a-time}
