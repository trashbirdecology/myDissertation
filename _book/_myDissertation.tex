...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{knitr}\OperatorTok{::}\NormalTok{opts_chunk}\OperatorTok{$}\KeywordTok{set}\NormalTok{(}\DataTypeTok{eval=}\OtherTok{FALSE}\NormalTok{)}
\CommentTok{# devtools::install_github("trashbirdecology/bbsAssistant",}
\CommentTok{#   dependencies = TRUE, force=FALSE)}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# a. Load the regional .txt file from }
\CommentTok{# Patuxent's FTP server (you must be connected to the }
\CommentTok{# internet to perform this step)}
...
\NormalTok{  regionFileName <-}\StringTok{ }\NormalTok{regions}\OperatorTok{$}\NormalTok{zipFileName }\OperatorTok{%>%}\StringTok{ }
\StringTok{    }\KeywordTok{na.omit}\NormalTok{()}
\NormalTok{  (regionFileName.use <-}
\StringTok{      }\NormalTok{regionFileName[}\KeywordTok{which}\NormalTok{(}\KeywordTok{str_detect}\NormalTok{(regionFileName, }\StringTok{"Flori"}\NormalTok{) }\OperatorTok{==}\StringTok{ }\OtherTok{TRUE}\NormalTok{)])}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# retrieve route-level data}
\NormalTok{routes <-}\StringTok{ }\KeywordTok{get_routeInfo}\NormalTok{() }
\CommentTok{# merge route-level data to bird count data}
\NormalTok{flBBS <-}\StringTok{ }\KeywordTok{left_join}\NormalTok{(flBBS, routes) }
...
\KeywordTok{glimpse}\NormalTok{(feather) }
\CommentTok{# Notice that the data imported from disk (feathers) }
\NormalTok{differs from the original BBS data }\ControlFlowTok{in}\NormalTok{ that the }
\CommentTok{# of columns is fewer (9 and 12 columns, respectively).}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{        bbsData <-}\StringTok{  }\KeywordTok{import_bbsData}\NormalTok{(}
            ...
            \DataTypeTok{RouteTypeID =} \DecValTok{1}
\NormalTok{        )}
...
\NormalTok{\}}\ControlFlowTok{else}\NormalTok{(}\KeywordTok{message}\NormalTok{(}\KeywordTok{paste0}\NormalTok{(}\StringTok{"NOT DOWNLOADING BBS DATA"}\NormalTok{))) }
      \CommentTok{# end if-else to download the data}
\end{Highlighting}
\end{Shaded}
