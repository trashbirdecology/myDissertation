...
```{r ewRoutesUsedHere, echo = FALSE, fig.align='center', out.width="95%", fig.cap = 'The three East-West running transects used to visualize results in this chapter.'}
...
```{r oneTsectEx, echo = FALSE, fig.align='center', out.width="95%", fig.cap = 'An example of two adjacent spatial transects within my sampling grid. After Fisher Ifnomration was calculated within a single transect, correlations were then calculated to determine the relationship among transect-pairs.'}
knitr::include_graphics(paste0(figDissDir, "/transectSamplingEx_1row.png"))
```
...
```{r adjacentTsectEx, echo = FALSE, fig.align='center', out.width="95%", fig.cap = 'An example of two adjacent spatial transects (12, 13) within my sampling grid. After Fisher Ifnomration was calculated within a single transect, correlations were then calculated to determine the relationship among transect-pairs.'}
knitr::include_graphics(paste0(figDissDir, "/transectSamplingEx_2rows.png"))
```
## Results  
### Fisher Information across spatial transects

Interpreting the Fisher Information is currently a qualitative effort. As suggested earlier, rapid increases or decreases in FI are posited indicate a change in system orderliness, potentially suggesting the location of a regime shift. Using this method yields inconclusive results regarding the location of 'spatial regimes' (Fig. \@ref(fig:fi1Tsect)). Of the three spatial transects analyzed in this chapter (Fig. \@ref(fig:ewRoutesUsedHere)), Fig.\@ref(fig:fi1Tsect) is representative of lack of pattern observed in teh Fisher Information values aross transects. I identified no clear pattern within or among spatial transects across space or time (Fig.\@ref(fig:usaFI)). Log-transforming the Fisher Information metric suppresses some of the extreme values, but still does not clearly identify sharp changes in the Fisher Information values. 
```{r fi1Tsect, echo = FALSE, out.width="95%", fig.cap="Fisher Information calculated for a single transect (number 15) over time."}
knitr::include_graphics(paste0(figDissDir,  "/transect_12_East-West_metric_FI_Eqn7.12.png"))
```
...
### Spatial correlation of Fisher Information
In addition to failing to identfiy clear geological boundaries across large swaths of our study area (Fig.\@ref(fig:usaFI)), I also did not identify spatial correlation of Fisher Information among adjacent spatial transects (Fig. \@ref(fig:corPlotTsectsInterp)). For spatially-adjacent transects (e.g, transects 11 and 12, or 12 and 13 in Fig. \@ref(fig:corPlotTsectsInterp)), we should expect high and positive correlation values, and these values shoudl stay consistent across time *unless* the spatial transects were separated by an East-West running physical or functional boundary. This is not, however, what I expect in our East-West running transects (Fig. \@ref(ewRoutesUsedHere)), as the spatial soft-boundaries limiting the distribution and functional potential of avian communities are largely North-South (Fig. \@ref(ewRoutes_ecoRegions)). 

```{r ewRoutes_ecoRegions,echo = FALSE,  out.width="95%", fig.cap="Routes used in this anlaysis laid against EPA Ecoregion Level 2 maps. Note spatial transects overlap multiple, large spatial ecoregion boundaries, such that we should expect our data to identify these sharp boundaries."}
...
```{r corPlotTsectsInterpLog, eval=FALSE,echo = FALSE, out.width="95%", fig.cap="Pairwise relationships of Fisher Information (interpolated values on the log-scale) of spatially adjacent transects over time. Pairs were compared (column) at select sampling years (rows), and pair-wise correlations among paired transects are presented. Large, positive correlations indicate Fisher Information signals similarly at adjacent spatial transects."}
knitr::include_graphics(paste0(figDissDir,  "/interpolated_FI_corplotSelectTransects_East-West_log.png"))
```

```{r fiSinglePair, echo = FALSE, out.width="95%", fig.cap="Fisher Information of two transect pairs over time. "}
knitr::include_graphics(paste0(figDissDir,  "/interp_FI_singlePair_corPlot_East-West.png"))
```

```{r fiEcoregion, echo = FALSE, out.width="95%", fig.cap="No patterns of abrupt change detected in Fisher Information along three transects in year 2010, and no obvious correlation with ecoregion boundaries (EPA Level 2). Size indicates value of Fisher Information (values are scaled and centered within transects). Red box (in top panel) indicates extent of bottom panel."}
knitr::include_graphics(paste0(figDissDir,  "/scaledFiInterpolated_year2010_zoom_East-West.png"))
```

Upon initial investigation, there are no obvious signs of broad-scale patterns in FI across space (Fig. \@ref(fig:fiEcoregion)). If Fisher Information is an indicator of spatial regime boundaries, we should expect to see large changes in its value (in either direction) near the edges of functional spatial boundaries (e.g., at the boundaries of ecoregions). No clear regime changes appeared in areas where we might expect rapid changes (e.g., along the 105th meridian West, where a sharp change in altitude occurs).
...
```{r fiEcoregion00, echo = FALSE, out.width="95%", fig.cap="Fisher Information (scaled and centered; point size positively correlated with value) along spatial transects, plotted against ecoregion boundaries (EPA Level 2)."}
