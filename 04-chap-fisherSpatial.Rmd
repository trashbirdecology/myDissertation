...
```{r bbsPoints,out.width="80%",  fig.cap = 'Locations of Breeding Bird Survey routes sampled between 1966 and 2017.'}
...
### Statistical signficance of Fisher Information

Although Fisher Information (FI) has been applied to multiple empirical datasets, inference from this metric still requires visual interpretation. Rapid changes in either direction of Fisher Information is suggested to be indicative of an impending regime shift [@mayer_fisher_2006, @eason_evaluating_  2012]. It follows that if some set of N points are close in space and are undergoing similar changes and are *not* separated a some boundary, which should ostensibly be identified in the FI metric, then these N points should exhibit similar (rapid or otherwise) changes in their FI values. 
```{r adjacentTsectEx, echo = FALSE, fig.align='center', fig.cap = 'A single East-West transect of Breeding Bird Survey routes used to calculate the Fisher Information binning measure. Military base locations can be used to visually estimate the proximity [of Department of Defense properties] to potential regime boundaries.'}
knitr::include_graphics(paste0(figDissDir, "/transectSamplingEx_2rows.png"))
```

I calculated the pairwise correlations of entire spatial transects (two at a time; see Fig. \@ref(fig:adjacentTsectEx)) at each year to determine the efficacy of using Fisher Information as an indicator of spatial boundary change.  

```{r calcFiCorrelations}
source("XXX.R")
```
...
Aside from the typical biases associated with the BBS data (e.g., species detection probability, observer bias, etc.), there are additional considerations to be made when using these data to identify 'spatial regimes'. BBS routes are spaced apart so as to reduce the probability of observing the same individuals (this is higly probable for most species songbird and accipiters, but birds whcih fly in large groups overhead to foraging or roosting sites have a higher pobabilty of being detected on multiple routes-- however we have removed these from this chapter analsysi). BEcause of this, we must assuem there is potential for each unique BBS route to represent its own state. If routes were closer together, it is more probable that the same type adn number of species would be identified on adjacent routes. Therefore, if this method does not detect slight changes in nearby routes which occupy the same 'regime', then it follows that the method is sensitive to loss or inclusion of new species, which are spatlly bounded by geological and vegetative characteristics. So waht new information does this give us about the system? This method reduces and removes the dimensionality of the middle-numbered system, losing critical information. Rather, would just identifying the potential spatial baoundaries based on abiotic and vegetative characterstics provide the same amount of inference for sometimes less money AND with more mechanistic insight? Using this method wuld only be useful if we suspect the system represents some emergent property.... which we cannot manage...? 
...
### More thoughts on FI
How does fisher infomration relate to likelihood ratio-based unsupervised algorithms (E.g., ChangeFinder [@liu2013change])?

What, if any, advantages does FI have over other density estimation techniques? e.g., 
