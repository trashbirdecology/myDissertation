...
# Setup -------------------------------------------------------------------
# Define teh location of the bbs data (which is already downloaded in another chapter)
...
# Download the  BBS data for KS + NE and save to file.. ------------------------------------

## Select the regions (states) we want to download
bbsRegions <- GetRegions() %>% 
  filter(stateName %in% c("NEBRASKA", "KANSAS"))
...
## Two state files should onlu take a couple fo minutes.
...
# Load in the BBS data and align with the sampling grid -------------------------------------------------------
...
## Subset the species by  removing waterfowl adn shorebirds
...
# UPload the AOU species codes --------------------------------------------

load(here::here("chapterFiles/discontinuityAnalysis/AOU_species_codes.RData"))
