...
# bbsAssistant: an R package for Download and Munging Data and Information from the North American Breeding Bird Survey {-#bbsAssistant}
...
![](./chapterFiles/appendices/regcred_select_trendests_byregion.png){width=300px}![](./chapterFiles/appendices/regcred_select_fl.png){width=300px}

Finally, copy the URL address for the page including the results and credibility measures (e.g. Florida):

![](./chapterFiles/appendices/regcred_select_treregcred_fl_ex.png){width=600px}
...
# regimeDetectionMeasures: an R package for calculating various regime detection measures {-#regimeDetectionMeasures}  
...
knitr::opts_chunk$set(echo=FALSE, eval=FALSE, message=FALSE, warning=FALSE, out.width = "85%")
...
# bbsRDM: an R package for applying `regimeDetectionMeasures` functionality to spatial data {-#bbsRDM}
...
```{r  bbsrdm_createDirs, echo=FALSE}
...
```{r bbsrdm_samplingGrid, echo=TRUE}
...
```{r bbsrdm_import.ind, echo=FALSE, eval=FALSE}
...
```{r bbsrdm_importfeathers,echo=TRUE}
if(import.feathers){
feathers <- NULL
featherNames <- list.files(bbsDir, pattern = ".feather")
featherNames <- str_c("/", featherNames) #add separator
...
```{r bbsrdm_subsetFxn, eval =FALSE, message = TRUE, warning = FALSE, echo=TRUE}
...
```{r bbsrdm_defineParams, echo=TRUE}
...
```{r bbsrdm_yearSubset, echo=FALSE, message=FALSE}
...
```{r bbsrdm_importCalcMetrics, echo=TRUE}
#  Import EWS results
...
```{r bbsrdm_importresults, echo=TRUE}
...
```{r bbsrdm_plotresults, echo=TRUE}
...
```{r bbsrdm.plotind.tsect, echo=TRUE}
...
<!-- ``` -->
# Functions used to calculate discontinuities in avian body mass distributions. {-#appDiscont}
## About
