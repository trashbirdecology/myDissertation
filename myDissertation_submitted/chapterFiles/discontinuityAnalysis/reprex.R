...
barFig3.dat.sim <-sort(barFig3.dat.sim)


plot(barFig3.dat.sim)
plot(density(barFig3.dat.sim))
...
for(i in seq_along(p)){
    ...
    # get space elapsed between observations (unit takes on unit log10.data)
    ...
    for (l in 1:length(ks)) {
        ...
        ## Identify the bandwidth on the cusp of being uni vs. muiltimodal
...
par(mfrow=c(1,2))
plot(density(Bootstrap.gaps$Bootstrap.gaps))
plot(density(Bootstrap.gaps$log10.data))
