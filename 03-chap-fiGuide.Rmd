...
```{r startParms, cache = TRUE, echo=FALSE, warning=FALSE, eval=TRUE, message=FALSE}
# Model parameters
parameters <- c(
  g1 = 1,
  m2 = 1,
...
```{r odeSolve, cache = TRUE, echo=FALSE, warning=FALSE, eval=TRUE, message=FALSE}
# require(deSolve)
# Vector of times
TT <-  11.145
...
1. The amount of information extracted from a set of measurements [@roy_frieden_physics_1998].        
1. A measure representing the dynamic order/organization of a system [@cabezas_towards_2002].       
...
The specified parameters for the model system are $g_1=m_2=1$, $l_12=g_12 = 0.01$ , $k=625$ ,and  $\beta=0.005$  [see @fath_regime_2003; @frieden_exploratory_2007; @mayer_applications_2007]. The initial conditions (predator and prey abundances) for the model system were not provided in the original references. Using package *deSolve* in Program R (v 3.3.2) to solve the model system \@ref(eq:predprey) I found $x_1  = 277.7815$ and $x_1= 174.551$ provided reasonable results. I found that a complete cycle of the system corresponds to approximately 11.145 time units. 

```{r pp1Period, cache = TRUE, echo=FALSE, warning=FALSE, eval=TRUE, message=FALSE, fig.cap=paste("Phase space plot of two-species Lotka-Volterra predator-prey system over a single period (~11.145 time units."), cache = T}
...
```{r 2D-hist, cache = TRUE, echo=FALSE, warning=FALSE, eval=TRUE, message=FALSE, fig.cap="A 2-dimensional histogram of the probability of observing a system in a particular state, $p(x)$, of the 2-species Lotka-Volterra predator prey system over a single period (~11.145 time units).", out.width="85%"}
...
```{r stringFig, cache = TRUE, echo=FALSE, warning=FALSE, eval=TRUE, message=FALSE, out.width="95%", fig.cap = paste("A single cycle of a hypothetical two-species system over time period $t = 0$ to $t = T$. $s^*$ is the state of the system at some point in time. The dotted line represents the distance travelled by the system in phase space over its trajectory during time $(0, T)$.")}
...
```{r solve-ODE, cache = TRUE, echo=FALSE, warning=FALSE, eval=TRUE, message=FALSE}
...
```{r FIcalc, cache = TRUE, echo=FALSE, warning=FALSE, eval=TRUE, message=FALSE}
# Equation 7.3b
p <- sysSol_highO$p
s <- sysSol_highO$s
dp <- lead(p)-p
...
```{r kSim, cache = TRUE, echo=FALSE, warning=FALSE, eval=TRUE, message=FALSE}
...
```{r dsdtOverTime, cache = TRUE, echo=FALSE, warning=FALSE, eval=TRUE, message=FALSE,out.width="85%", fig.cap=paste("Speed of the system (rate of change) in phase space. Dashed vertical line at time 200 indicates location of regime shift.")}
