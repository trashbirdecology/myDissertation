...
In many cases the standard deviation of FI exceeded the mean value of FI as the proportion of data (and observations) used decreased. For example, when when we retained 
...
When we bootstrap 25% of the species, the ratio of mean Fisher Information to standard deviation of Fisher Information (over `r 1e4` iterations) is always `r < 1`, suggesting Fisher Information does not produce fidel results when information is lost about the system. 
```{r results, fig.align='center', out.width = "85%",  echo=FALSE,  fig.cap="Density plot of the ratio of mean:standard deviation of the Fisher Information when boostrapping 25% of the species (over 1000 iterations)."}
knitr::include_graphics(loadFig("fiRatio_densityPlot"))
```
