...
To account for unequal time elapsed between observations (i.e., irregular sampling), we must next correct for change in time. Since $\Delta s$ is now the total change in the system between two time points, we can simply correct for variability in change in time, $\Delta t$, by dividing $\Delta s$ by $\Delta t$. I refer to this new measure as **system velocity**:
