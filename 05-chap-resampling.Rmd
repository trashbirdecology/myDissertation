...
In many cases the standard deviation of FI far exceeded the mean value of FI. I calculated the coeficient of variation (standard deviation divided by the mean), $frac{}{}$, for each . 

as the proportion of data (and observations) used decreased. For example, when when we retained 
### Velocity of the distance travelled produces similar results with information loss

### Variance Index produces 

### Fisher Information is highly sensitive to information loss 

When we bootstrap 25% of the species, the ratio of mean Fisher Information to standard deviation of Fisher Information (over `r 1e4` iterations) is always $< 1$, suggesting Fisher Information does not produce fidel results when information is lost about the system. 
```{r results, fig.align='center', out.width = "85%",  echo=FALSE,  fig.cap="Density plot of the coefficient of variation (CV) as a percentage (%) of the Fisher  Information bootstrapped samples (10,000 iterations). Densiies based on all values of CV, but values $>100%$ are not printed."}
