...
# Chunk defaults
# require(knitr)
knitr::opts_chunk$set(cache = TRUE, cache.path = "_cache/", message = FALSE, warning=FALSE, eval=TRUE, echo=FALSE, error=FALSE)
get_os <- function(){
  sysinf <- Sys.info()
  if (!is.null(sysinf)){
    os <- sysinf['sysname']
...
```{r timeElapsed, echo=FALSE, fig.cap="The amount of time elapsed between observations.", out.width="85%"}
...
```{r dsdtResamp,   echo=FALSE, out.width="85%", fig.cap="Mean velocity and associated 95% confidence intervals over 10,000 iterations using the observations resampling method. Red line indicates the value of velocity when **M** and **P** = 100%."}
knitr::include_graphics(loadFig("dsdt_observations_ribboned_facetByProb")) # observations method
```

\pagebreak[4]  

```{r dsdtResampRegime2,   echo=FALSE, out.width="85%", fig.cap="Mean velocity and associated 95% confidence intervals over 10,000 iterations using the observations resampling method for a subset of the time series (the second 'regime' identified). Red line indicates the value of velocity when **M** and **P** = 100%."}
...
\begin{landscape}
```{r loessEx,  echo=FALSE, out.width="85%", fig.cap="Local regression (loess) smoothing of a dominant species in the paleodiatom community, \textit{Anomoeoneis costata} varies with the span parameter, making it difficult to justify smoothing the data prior to calculating various regime detection metrics."}
knitr::include_graphics(loadFig("FI_cvDensity"))
```
\end{landscape}
