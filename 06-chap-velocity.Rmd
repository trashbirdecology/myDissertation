...
(\#eq:velocity)
\end{equation}
```{r velocSysEx1, echo=FALSE, message=FALSE, warning=FALSE, fig.cap = "System change ($s$) and velocity ($v$) of the model system over the time period. Constant means ($\\bar{x}_{pre}=25$, $\\bar{x}_{post}=10$) and sharp change in variance for both state variables, $\\sigma =5$."}
x_1 = c(rnorm(mean =25, sd = 5, n=50), rnorm(mean =100, sd = 5, n=50))
x_2 = c(rnorm(mean =25, sd = 5, n=50), rnorm(mean =100, sd = 5, n=50))
t = 1:length(x_1)
df.wide = data.frame(t, x_1, x_2) 
...
```{r simVplot1, eval=TRUE, fig.cap = "Velocity ($v$) generally increases as the total change in the mean value of $\\bar{x}_{1_{t=50}}$ increases in a single iteration of our toy system ($N_{iter}=1$, seed = 123). This 2-variable system exhibits a regime shift at $t=50$, where variance is constant $\\sigma = 5$, $\\bar{x}_1 = 25$ when $t<50$,  $\\bar{x}_2 = 50$ when $t \\geq50$, $\\bar{x}_1 = 25$ when $t <50$."}
...
```{r simVarPlot, eval=TRUE, fig.cap = "High variance of velocity ($v$) in a single iteration ($N_{iter}=1$, seed = 123) of simulations as we increase $\\sigma_1$ at $t=50$."}
ggplot(data = dist.sim %>% filter(sim.num==1, t==51), 
...
```{r simVarPlot2,  cache = TRUE, echo=FALSE, warning=FALSE, eval=TRUE, message=FALSE, fig.cap = "Average ($\\pm 2$ SD) velocity ($v$) woresens as the variance of $\\bar{x}_{2_{t=50 (post)}}$ (post shift) increases. $\\bar{x}_{1_{pre}} = 25$, $\\bar{x}_{1_{post}} = 100$, $\\bar{x}_{2_{pre}} = 25$, $\\bar{x}_{2_{post}} = 50$, $\\sigma_{1_{pre}} = 5$, $\\sigma_{2_{pre,post}} = 5$"}
ggplot(data = dist.sim.reduced) +
...
### Performance on empirical data: paleodiatom community example
...
```{r paleoTurnover, echo=F, warning=F, message=F, cache = T, fig.cap="Relative abundances of the most common diatom species in the time series. Few species dominate the data over the entire time series, and turnover is apparent at multiple observations."}
topSpecies <-
  paleo %>%
  group_by(variable) %>%
...
```{r paleoVelocity,  echo=F, warning=F, message=F, cache = T, fig.pos="T", fig.cap ="Velocity $v$ and distance travelled $s$ of the paleodiatom time series. Dashed line at 1,300 years before 1950 indicates the regime shift identifed in Spanbauer et al. (2014). Dotted lines indicate regime shifts identified using $v$ and $s$."}
...
ggplot(plotData, aes(x = sortVar*-1, y = value))+
  ...
  theme(panel.grid.minor = element_blank(),
     ...
     scale_x_reverse()+
  geom_vline(xintercept = 1300, color="red",linetype=2)+
  geom_vline(xintercept = c(1383, 2150, 4800), color="blue",linetype=3)
```
...
```{r paleoRegime1and3,  echo=F, warning=F, message=F, cache = T, fig.pos="T", fig.cap ="Velocity $v$ indicates periodic-like conditions in the first (*a*) and second (*b*) regimes."}
...
```{r paleoObsPred, eval=TRUE, cache = TRUE, echo=FALSE, warning=FALSE,  message=FALSE, fig.cap = "The regularized differentiation of $s$ ($pred$) was best fit using $\alpha = 100$. Higher overlap of $s$ and $pred$ indicate a good fit of the regularized differentiated metric to the non-smoothed metric, $s$."}
...
```{r paleoV, cache = TRUE, echo=FALSE, warning=FALSE, eval=TRUE, message=FALSE}
# Plot derivative
ggplot(paleoEx %>% 
         filter(key == "dsdt"), 
       aes(x = t*-1, y = value)) +
