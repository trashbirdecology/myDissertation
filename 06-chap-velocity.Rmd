...
```{r lorenz3Dts, eval=TRUE, echo=FALSE,    out.width="85%", fig.cap="An example solution of the Lorenz ('butterfly') represented in individual system compoents."}
...
```{r paleoV,  eval=TRUE, cache = TRUE, echo=FALSE, warning=FALSE, message=FALSE, out.width="85%", fig.cap="The velocity metric  ($v$) signals potential periodicities in the paleo diatom time series data when the distance traveled metric, $s$, is smoothed using regularized differentiation methods [@price2019tvdiff]."}
# Plot derivative
# p=ggplot(paleoEx %>% 
#          filter(key == "dsdt"), 
#        aes(x = t*-1, y = value)) +
