...
```
To determine whether the rapid shift in $v$ observed in the toy system is sensitve to the total change in $\bar{x}_{1_{pre}}$ and  $\bar{x}_{1_{post}}$ (Fig. \@ref(fig:velocSysEx1)), I simulated $1,000$ iterations of the toy system across a range of $\bar{x}_{1_{post}}$ (from $\bar{x}_1 = 15$ to $\bar{x}_1 = 100$, at intervals of $5$). As expected, the magnitude of $v$ increased linearly as the total difference between $\bar{x}_{1_{pre}} and \bar{x}_{1_{post}}$ increased (\@ref(fig:simVplot2)). This is not surprising, as $s$ increases as the total change in abundance across the entire sytem increases  (Eq. \@ref(eq:s)), therefore, the potential maximum of $v$ also increases. This may indicate that $v$, while capable of identifying large shifts in data structure, may not pick up subtle changes (i.e. lower effect sizes). 
...
Further, I also examined the sensitivity of $v$ to changes in variance $\sigma x_i$ at the shift location. Unsurprisingly, $v$ appears senstive to 
```{r simVar}
...
```{r simVarPlot, eval=FALSE, fig.cap = "Velocity ($v$$) generally increases as the total change in the variance of $\\x_{1_{t=50}}$ increases. This 2-variable system exhibits a regime shift at $t=50$."}
ggplot(data = dist.sim %>% filter(sim.num==666, t==51), 
       aes(x = as.integer(var.sim), y = dsdt))+
  geom_line()+
  theme_bw()+
  ylab(expression(paste("velocity, ",italic("v"), " at ", italic("t = 51"))))+
  xlab(expression(paste("total change in ", "sigma"[1], " at ", italic("t = 50"))))
```
