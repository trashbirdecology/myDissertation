...
```{r tvDiffPaleoCalculate, cache = TRUE, echo=FALSE, warning=FALSE, eval=TRUE, message=FALSE, results="hide"}
library(tvdiff)
library(tidyverse)
library(dplyr)
library(ggplot2)
...
paleoInterp <- paleoInterp %>%
  unnest()
```
```{r tvDiffPaleoCalculate2, cache = TRUE, echo=FALSE, warning=FALSE, eval=TRUE, message=FALSE, results="hide"}
# Calculate distance
paleoDistance <- 
  paleoInterp %>% 
  group_by(variable) %>% 
  ...
  na.omit()
```
```{r tvDiffPaleoCalculate3, cache = TRUE, echo=FALSE, warning=FALSE, eval=TRUE, message=FALSE,results="hide"}
# Build dataframe
paleoEx <-
  paleoDistance %>% 
  mutate(dt = lead(t) - t) %>% 
...
```{r paleoV,  eval=TRUE, cache = TRUE, echo=FALSE, warning=FALSE, message=FALSE, out.width="75%", fig.cap="The velocity metric  ($V$) signals potential periodicities in the paleo diatom time series data when the distance traveled metric, $s$, is smoothed using regularized differentiation methods (see Price and Burnett 2019)."}
# Plot derivative
# p=ggplot(paleoEx %>% 
#          filter(key == "dsdt"), 
#        aes(x = t*-1, y = value)) +
