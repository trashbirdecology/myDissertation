...
```{r lorenz3Dts, eval=TRUE, echo=FALSE,    out.width="75%", fig.cap="An example solution of the Lorenz ('butterfly') represented in individual system compoents."}
...
```{r simVarPlot2,  cache = TRUE, echo=FALSE, warning=FALSE, eval=TRUE, message=FALSE,   out.width="75%", fig.cap = "Average ($\\pm 2$ SD) velocity ($v$) worsens as the variance of $\\bar{x}_{2_{t=50 (post)}}$ (post shift) increases. $\\bar{x}_{1_{pre}} = 25$, $\\bar{x}_{1_{post}} = 100$, $\\bar{x}_{2_{pre}} = 25$, $\\bar{x}_{2_{post}} = 50$, $\\sigma_{1_{pre}} = 5$, $\\sigma_{2_{pre,post}} = 5$"}
...
```
```{r paleoRegime1and3,  echo=FALSE, warning=F, message=F, cache = T,    out.width="75%", fig.cap ="Velocity ($v$) indicates periodic-like conditions in the first (A) and second (B) regimes."}
...
```{r velocSysEx2,message=FALSE, warning=FALSE, echo=FALSE,   out.width="33%", fig.cap = "System change ($s$) and velocity ($v$) of the model system over the time period. Change in means ($\\bar{x}_{1_{pre}}=25$, $\\bar{x}_{1_{post}}=100$, $\\bar{x}_{2_{pre}}=50$, $\\bar{x}_{2_{post}}=10$) and an increase in variance ($\\sigma_{1_{pre}}=2$, $\\sigma_{1_{post}}=10$, $\\sigma_{2_{pre}}=5$,  $\\sigma_{2_{post}}=10$)."}
...
```{r velocSysEx3, message=FALSE, warning=FALSE, echo=FALSE,   out.width="33%", fig.cap = "System change ($s$) and velocity ($v$) of the model system over the time period. Constant means ($\\bar{x}_1=25$, $\\bar{x}_2=50$) and sharp change in variance for one state variable $\\sigma_{1_{pre}} = 2$, $\\sigma_{1_{post}} = 12$, $\\sigma_{2_{pre,post}} = 5$"}
...
```{r velocSysEx4, message=FALSE, warning=FALSE, echo=FALSE,   out.width="33%", fig.cap = "System change ($s$) and velocity ($v$) of the model system over the time period. Variance equal to mean ($/bar{x_i}=/sigma_i$), where means ($/bar{x}_{1_{pre}}=25$, $/bar{x}_{1_{post}}=50$, $/bar{x}_{2_{pre}}=15$, $/bar{x}_{2_{post}}=150$)."}
